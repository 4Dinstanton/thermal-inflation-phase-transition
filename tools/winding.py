...
Uses float32 and incremental plaquette sums to limit peak RAM on 1024³ grids.
"""
import numpy as np
...
def compute_winding_number(phi1, phi2, out=None, dtype=np.float32):
    ...
    dtype : numpy dtype
        Working precision (float32 recommended for N>=512).
    ...
    theta = np.arctan2(phi2, phi1, dtype=dtype)
    if out is None:
        w = np.zeros_like(theta, dtype=dtype)
    else:
        w = np.asarray(out, dtype=dtype)
        w[...] = 0.0

    # One plaquette orientation at a time — avoid holding 6 full theta rolls.
    th_ip = np.roll(theta, -1, axis=0)
    th_jp = np.roll(theta, -1, axis=1)
    w += _plaquette_winding_arr(theta, th_ip, np.roll(th_ip, -1, axis=1), th_jp)
    del th_ip, th_jp

    th_ip = np.roll(theta, -1, axis=0)
    th_kp = np.roll(theta, -1, axis=2)
    w += _plaquette_winding_arr(theta, th_ip, np.roll(th_ip, -1, axis=2), th_kp)
    del th_ip, th_kp

    th_jp = np.roll(theta, -1, axis=1)
    th_kp = np.roll(theta, -1, axis=2)
    w += _plaquette_winding_arr(theta, th_jp, np.roll(th_jp, -1, axis=2), th_kp)
    del th_jp, th_kp, theta
