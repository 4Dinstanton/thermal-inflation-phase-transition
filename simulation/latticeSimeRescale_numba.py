...
parser.add_argument(
    ...
    help="Integrator kernel (default: rk2_nonfused = full 2nd-order Langevin with table V'). "
    "'overdamped' uses first-order Langevin (no momentum, must be explicitly selected). "
    "'baoab' uses BAOAB splitting (also full 2nd-order). "
    "'rk2_nonfused' is the recommended default for physical accuracy.",
)
parser.add_argument(
    "--phi_threshold",
    type=float,
...
USE_BAOAB = False  # BAOAB splitting integrator: 2 passes (~2x faster)
...
# Overdamped mode is now opt-in only (use --integrator overdamped).
# No auto-switching based on eta_phys.
...
def compute_energy_densities(phi, pi, dx, inv_a2, lam_val, mphi_val, mu):
    ...
    Returns (E_kin, E_grad, E_pot) as mean values per site in rescaled units.
    E_kin  = (1/2) <pi^2>
    E_grad = (1/2) inv_a2 <|forward_grad phi|^2> / dx^2
    E_pot  = <V_tree(phi)> / mu^2   where V_tree = lam/4 phi^4 - m^2/2 phi^2
    """
    nx, ny, nz = phi.shape
    n_sites = nx * ny * nz
    inv_dx2 = 1.0 / (dx * dx)
    ...
    sum_kin = np.float64(0.0)
    sum_grad = np.float64(0.0)
    sum_pot = np.float64(0.0)

    for i in nb.prange(nx):
        ip = i + 1 if i + 1 < nx else 0
        local_kin = np.float64(0.0)
        local_grad = np.float64(0.0)
        local_pot = np.float64(0.0)
        for j in range(ny):
            jp = j + 1 if j + 1 < ny else 0
            for k in range(nz):
                kp = k + 1 if k + 1 < nz else 0
                ph = np.float64(phi[i, j, k])
                pi_v = np.float64(pi[i, j, k])

                local_kin += 0.5 * pi_v * pi_v

                dx_f = np.float64(phi[ip, j, k]) - ph
                dy_f = np.float64(phi[i, jp, k]) - ph
                dz_f = np.float64(phi[i, j, kp]) - ph
                local_grad += (
                    0.5 * inv_a2 * (dx_f * dx_f + dy_f * dy_f + dz_f * dz_f) * inv_dx2
                )

                ph2 = ph * ph
                local_pot += (
                    0.25 * lam_val * ph2 * ph2 - 0.5 * mphi_sq * ph2
                ) * inv_mu2
...
if USE_OVERDAMPED:
    print(f"Integrator: Overdamped (first-order) Langevin, Heun method")
    print(f"  η = {eta_phys} GeV — no momentum (pi), pure dissipative dynamics")
elif OVERDAMPED_MODE:
    print(f"Overdamped mode: ENABLED (η = {OVERDAMPED_ETA}, no ringing)")
else:
    print(f"Integrator: Full 2nd-order Langevin ({INTEGRATOR_NAME})")
    print(f"  EOM: ∂²φ/∂t² + η ∂φ/∂t = ∇²φ − V'(φ) + ξ")
    print(f"  η = {eta_phys} GeV (rescaled: {eta:.4f})")
if HUBBLE_EXPANSION:
...
for n in range(n_start, Nt):
    ...
    if _should_save:
        ...
        # Energy diagnostics (optional)
        if cli_args.diag_energy and not USE_OVERDAMPED:
            _e_kin, _e_grad, _e_pot = compute_energy_densities(
                phi, pi, dx, inv_a2, lam, mphi, mu
            )
            _e_tot = _e_kin + _e_grad + _e_pot
            _e_kin_expect = mu * mu * T / (2.0 * dx_phys**3)
            _equip_ratio = _e_kin / _e_kin_expect if _e_kin_expect > 0 else 0.0
            print(
                f"  Energy: E_kin={_e_kin:.4e}  E_grad={_e_grad:.4e}  "
                f"E_pot={_e_pot:.4e}  E_tot={_e_tot:.4e}  "
                f"E_kin/equip={_equip_ratio:.4f}"
            )
        ...
        if cli_args.diag_energy and not USE_OVERDAMPED:
            save_dict["E_kin"] = _e_kin
            save_dict["E_grad"] = _e_grad
            save_dict["E_pot"] = _e_pot
...
if REPLAY_MODE:
    ...
    print("\nOptimizations Used:")
    if USE_OVERDAMPED:
        print(f"  Integrator: OVERDAMPED Heun (1st-order Langevin, 2 passes/step)")
    elif USE_BAOAB and USE_VPRIME_TABLE:
        print(f"  Integrator: BAOAB (full 2nd-order Langevin, 2 passes/step)")
    elif USE_VPRIME_TABLE and USE_NONFUSED_TABLE_RK2:
        print(f"  Integrator: RK2 table (full 2nd-order Langevin, 2 passes/step)")
    else:
        print(f"  Integrator: {INTEGRATOR_NAME}")
