...
def main():
    ...
    T_sp_lo, T_sp_hi, N_sp = 6600.0, 6780.0, 1000
    ...
    #   => T_c ~ O(1e7) GeV
    # ----------------------------------------------------------------
    T_c_lo, T_c_hi, N_c = 1.0e7, 5.0e7, 1000
    ...
    if T_c is not None:
        ...
        print(f"         phi_min(T_c)          = {phi_min_Tc:.6e} GeV  ({phi_min_Tc/1e3:.2f} TeV)")
    else:
    ...
    if T_sp is not None:
        ax1.axvline(T_sp, color="red", ls="--", lw=1.2,
                     label=f"$T_{{sp}}$ = {T_sp:.2f} GeV")
    ...
    if T_c is not None:
        ax2.axvline(T_c, color="red", ls="--", lw=1.2,
                     label=f"$T_c$ = {T_c:.1f} GeV")
    ...
    fig.suptitle("Critical Temperatures (fermion_only, set6)", fontsize=13)
    fig.tight_layout()
    fig.savefig("figs/finiteTemp/critical_temperatures.png", dpi=200,
                bbox_inches="tight")
    ...
    if T_sp is not None:
        ...
        fig_sp.tight_layout()
        fig_sp.savefig("figs/finiteTemp/potential_barrier_disappearance.png",
    ...
    if T_c is not None:
        ...
        for T_plot, lab in tc_temps_bg:
            ...
            axes_tc[0].plot(phi_full.ravel() / 1e3, V_vals,
                            label=lab, lw=1.2, alpha=0.7)
            phi_m, V_m = find_true_minimum(VT, vev_0)
            col = axes_tc[0].get_lines()[-1].get_color()
            axes_tc[0].plot(phi_m / 1e3, V_m - V_zero, "o", color=col,
                            ms=5, zorder=5)
        ...
        V_vals = VT.V_p_fermion_only(phi_full).ravel() - V_zero
        axes_tc[0].plot(phi_full.ravel() / 1e3, V_vals,
                        label=lab, lw=2.5, color="black", zorder=10)
        phi_m, V_m = find_true_minimum(VT, vev_0)
        axes_tc[0].plot(phi_m / 1e3, V_m - V_zero, "o", color="black",
                        ms=8, zorder=11)
        ...
        for T_plot, lab in tc_temps_bg:
            ...
            axes_tc[1].plot(phi_vev_zoom.ravel() / 1e3, V_vals,
                            label=lab, lw=1.2, alpha=0.7)
            phi_m, V_m = find_true_minimum(VT, vev_0)
            col = axes_tc[1].get_lines()[-1].get_color()
            axes_tc[1].plot(phi_m / 1e3, V_m - V_zero, "o", color=col,
                            ms=5, zorder=5)
        ...
        axes_tc[1].plot(phi_vev_zoom.ravel() / 1e3, V_vals,
                        label=lab, lw=2.5, color="black", zorder=10)
        phi_m, V_m = find_true_minimum(VT, vev_0)
        axes_tc[1].plot(phi_m / 1e3, V_m - V_zero, "o", color="black",
                        ms=8, zorder=11)

        axes_tc[1].axhline(0, color="red", ls="--", lw=1.0, alpha=0.7,
                            label="V(0) level")
        ...
        fig_tc.tight_layout()
        fig_tc.savefig("figs/finiteTemp/potential_vacuum_degeneracy.png",
                        dpi=200, bbox_inches="tight")
